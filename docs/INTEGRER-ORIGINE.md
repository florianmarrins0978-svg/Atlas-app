...
**`scroll-snap-stop: always` fait « un chantier à la fois » — et l'application
ne le porte plus.** Retiré le 11 août 2026 : couper un geste vif à chaque
chantier se lit comme du saccadé, et c'est le mot que le patron a employé.
L'accroche est passée en `proximity`. Cette maquette garde la règle d'origine ;
le code fait foi (`src/app/globals.css`, `PROJECT_STATE.md`).
