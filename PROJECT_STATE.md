...
**Dernière mise à jour :** 2026-08-12 · branche `claude/calendar-app-connection-lkowne`
...
| **L'agenda iCloud, dans les deux sens** — Atlas lit les créneaux occupés du compte Apple et **n'y propose plus** de date ; s'il l'allume, il pose ses chantiers dans le calendrier qu'il désigne et les retire quand il déplanifie. Les deux fournisseurs se fondent en une seule carte d'occupation. **Réserve : aucun échange réel avec iCloud n'a eu lieu ici** (réseau refusé) — voir `ARCHITECTURE.md` §73 | `src/lib/ics.ts` + `src/lib/caldav.ts` + `src/server/agenda/apple.ts` + `src/server/repositories/agenda-apple.ts` + `drizzle/0035_agenda_apple.sql` |
...
- **Agenda iCloud** — demandé le 12 août 2026 **dans les deux sens**, et
  **codé le jour même** : lecture des créneaux occupés, écriture des chantiers,
  retrait au débranchement (`ARCHITECTURE.md` §73). Ce qui **reste** : aucun
